...
*   **目标函数 (Minimize)**：
    ...
    其中 $N$ 是层数，$b_i$ 是第 $i$ 层选择的比特数，$S_i(b_i)$ 是该层在该比特下的敏感度得分（如 DeltaLoss）。

*   **约束条件 (Subject to)**：
    $$ \frac{\sum_{i=1}^{N} P_i \times b_i}{\sum_{i=1}^{N} P_i} \le \text{Target Avg Bits} $$ 
    其中 $P_i$ 是第 $i$ 层的参数量（权重总数）。
...
5.  **调整边界**：
    - 若 $\text{Avg}_{\text{current}} > \text{Target}$：价格太低了，导致超支。调高价格：$\lambda_{\min} = \lambda_{mid}$。
    - 若 $\text{Avg}_{\text{current}} < \text{Target}$：价格太高了，过于节省。调低价格：$\lambda_{\max} = \lambda_{mid}$。
6.  **迭代**：重复步骤 2-5，直至收敛或达到迭代次数上限。
